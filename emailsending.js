...
function getEmailBody(company, stepp, theme = 'dark') {
  ...
  const ctaHrefTrial =
    `${PUBLIC_BASE_URL}/api/clk?rid={{trackingId}}&to=${encodeURIComponent(`${rawCtaTrial}${rawCtaTrial.includes('?') ? '&' : '?'}rid={{trackingId}}`)}`;
  const unsubHref =
    `${PUBLIC_BASE_URL}/api/clk?rid={{trackingId}}&to=${encodeURIComponent(`${rawUnsubscribe}${rawUnsubscribe.includes('?') ? '&' : '?'}rid={{trackingId}}`)}`;
...
.cta {
  ...
  background:${T.ctaBg};color:${T.ctaText}!important;
  border-radius:8px;border:1px solid ${T.ctaBd};
...
<td class="content">
${bodyInnerHtml}
<a href="${ctaHrefTrial}" class="cta" target="_blank" rel="noopener">${escapeHtml(ctatext)}</a>
</td>
</tr>
...
alt="Tamara Gangsøy" width="128" style="border-radius:10px;display:block;" />
</td>
...
<p style="margin:0 0 10px;">Kundeservice<br><strong>+47&nbsp;45&nbsp;49&nbsp;19&nbsp;01</strong><br>
<a href="mailto:tamara@innkjops-gruppen.no" style="color:${T.link};text-decoration:none;">tamara@innkjops-gruppen.no</a></p>
</td>
</tr>
</table>
...
<div class="footer">
<span class="mute">Ønsker du ikke flere e-poster?</span>&nbsp;<a class="unsub" href="${unsubHref}" target="_blank" rel="noopener">Avmelding</a>
</div>
