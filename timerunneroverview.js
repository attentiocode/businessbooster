...
const timeRunner = (() => {
  ...
  function loading(){
    ...
    STATE.el.innerHTML = headerHtml() + `
      <div class="tro-wrap">
        ${Array.from({length:7}).map(()=>skeletonCard()).join('')}
  ...
  function computeMetrics(data){
    ...
    const t = data.reduce((acc,row) => {
      ...
      if(isTrue(row.accepted)) acc.accepted++;    // <-- NEW

      return acc;
    }, { total:0, sent:0, processing:0, nextMonth:0, stopped:0, accepted:0 });
  ...
  function animateAll(curr, prev){
    ...
    prev = prev || {
      leads:0,total:0,sent:0,processing:0,nextMonth:0,stopped:0,accepted:0,progress:0,sentPct:0
    };
    ...
    animateNumber(document.getElementById('tro-acc'),      prev.accepted,  curr.accepted,  dur); // NEW
    ...
    animatePct(  document.getElementById('tro-sent-pct'),
                 percentSafe(prev.sent, prev.total),
                 percentSafe(curr.sent, curr.total),
                 dur);
